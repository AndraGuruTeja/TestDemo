...
#     await redis.zadd("user_activity", {user.id: datetime.now().timestamp()})
#     return user

# # Dynamic rate limiter
# async def dynamic_rate_limiter(
#     request: Request,
#     current_user: User = Depends(get_current_user),
#     redis: aioredis.Redis = Depends(get_redis)
# ):
#     # Track user activity
#     await redis.zadd("user_activity", {current_user.id: datetime.now().timestamp()})
    
#     # Calculate active users (last 5 minutes)
#     now = datetime.now().timestamp()
#     active_users = await redis.zcount("user_activity", now - 300, now)
    
#     # Calculate dynamic limits
#     BASE_LIMIT = 100
#     LIMIT_PER_ACTIVE_USER = 5
#     dynamic_limit = BASE_LIMIT + (active_users * LIMIT_PER_ACTIVE_USER)
    
#     # Per-user rate limiting
#     rate_key = f"user:{current_user.id}:{request.url.path}"
...
# # Weather endpoints with dynamic rate limiting
# @app.get("/weather/{city}", response_model=WeatherResponse, dependencies=[Depends(dynamic_rate_limiter)])
# async def get_weather(
#     city: str,
#     db: Session = Depends(get_db),
#     current_user: User = Depends(get_current_user)
...
# @app.get("/weather/history/{city}", response_model=List[HistoricalWeatherResponse], dependencies=[Depends(dynamic_rate_limiter)])
# async def get_history(
#     city: str,
#     days: int = 7,
#     db: Session = Depends(get_db),
...
# @app.get("/weather/trends/{city}", response_model=List[WeatherTrendResponse], dependencies=[Depends(dynamic_rate_limiter)])
# async def get_trends(
#     city: str,
#     days: int = 7,
#     db: Session = Depends(get_db),
