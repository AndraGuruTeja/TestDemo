...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*"],  # Allow all origins (update to specific origins in production)
...
#     try:
#         # Get real-time data from OpenWeather API
...
async def login(
    ...
    db: Session = Depends(get_db),
    rate_limiter: RateLimiter = Depends(RateLimiter(times=100, seconds=60))
):
    user = db.query(User).filter(User.email == form_data.username).first()
    if not user or not pwd_context.verify(form_data.password, user.hashed_password):
        raise HTTPException(status_code=401, detail="Invalid credentials")
